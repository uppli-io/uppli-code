...
fn build_display_lines(screen: &SettingsScreen) -> Vec<Line<'static>> {
    ...
    lines.push(indent_line(
        "  Options: default, dark, light  (use /theme to change)",
        Color::DarkGray,
    ));
    lines.push(Line::from(""));
...
impl PrivacySnapshot {
    /// Load privacy fields from `~/.uppli/settings.json`.
    fn load() -> Self {
        let path = cc_core::config::Settings::config_dir().join("settings.json");
...
fn build_privacy_lines(screen: &SettingsScreen) -> Vec<Line<'static>> {
    ...
    lines.push(Line::from(vec![Span::styled(
        "  These settings control data sharing with your provider.",
        Style::default().fg(Color::DarkGray),
    )]));
    lines.push(Line::from(""));
    ...
    lines.push(indent_line(
        "  Whether you have agreed to the usage policy.",
        Color::DarkGray,
    ));
    lines.push(Line::from(""));
    ...
    privacy_toggle_lines(
        ...
        "Sends anonymised usage statistics to help improve Uppli Code.",
    );
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(vec![Span::styled(
        "  Note: Edit ~/.uppli/settings.json to toggle telemetry/sharing values.",
        Style::default()
            .fg(Color::Yellow)
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(vec![Span::styled(
        "  All data stays local. API calls go to DeepSeek only.",
        Style::default().fg(Color::DarkGray),
    )]));
