...
//! Data source: ~/.uppli/stats.jsonl (append-only per-turn usage log)
...
/// A single entry in ~/.uppli/stats.jsonl
...
/// Load and aggregate stats from ~/.uppli/stats.jsonl
pub fn load_stats() -> AggregatedStats {
    let path = dirs::home_dir()
        .map(|h| h.join(".uppli").join("stats.jsonl"))
        .unwrap_or_default();
