// cc-tui: Terminal UI using ratatui + crossterm for the Uppli Code Rust port.
//
...
pub mod input;
/// Startup dialog for malformed settings.json or UPPLI.md.
pub mod invalid_config_dialog;
/// MCP server management UI.
pub mod mcp_view;
/// Memory file selector overlay (UPPLI.md browser).
pub mod memory_file_selector;
/// Memory update notification banner (shown after Claude updates a UPPLI.md file).
...
pub use model_picker::{render_model_picker, EffortLevel, ModelEntry, ModelPickerState};
...
mod tests {
    ...
    fn test_agents_editor_ctrl_s_saves_new_agent() {
        ...
        let saved = temp.path().join(".uppli").join("agents").join("planner.md");
        assert!(saved.exists());
    ...
    fn test_render_app_keeps_logo_header_after_first_message() {
        ...
        assert!(rendered.contains("Uppli Code"));
