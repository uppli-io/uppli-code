...
fn render_tool_use_inner(tool_name: &str, input: &serde_json::Value) -> Vec<Line<'static>> {
    ...
    let mut header_spans = vec![
        Span::styled("  \u{25cf} ".to_string(), Style::default().fg(Color::Green)),
        Span::styled(
            tool_name.to_string(),
            Style::default()
                .fg(Color::White)
...
pub fn render_rate_limit_with_hint(
    ...
    if show_upgrade_hint {
        lines.push(Line::from(vec![Span::styled(
            "  \u{2192} uppli.fr for higher limits",
            Style::default().fg(Color::DarkGray),
        )]));
    }
    lines
...
mod tests {
    ...
    fn test_render_user_memory_input() {
        let result = render_user_memory_input("project", "Uppli Code Rust Port");
        assert_eq!(result.len(), 2);
        let first = line_text(&result[0]);
        assert!(first.contains("# project: Uppli Code Rust Port"));
