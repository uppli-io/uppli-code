...
/// A defined agent (from .uppli/agents/*.md or plugin).
#[derive(Debug, Clone)]
...
/// Load agent definitions from `.uppli/agents/` in project root and home dir.
pub fn load_agent_definitions(project_root: &std::path::Path) -> Vec<AgentDefinition> {
    let mut defs = Vec::new();
    let dirs = [
        dirs::home_dir().map(|h| h.join(".uppli").join("agents")),
        Some(project_root.join(".uppli").join("agents")),
    ];
