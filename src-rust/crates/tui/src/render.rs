...
const UPPLI_ACCENT: Color = Color::Rgb(233, 30, 99);
...
fn startup_notice_lines(app: &App, width: u16) -> Vec<Line<'static>> {
    ...
    match &app.bridge_state {
        crate::bridge_state::BridgeConnectionState::Connected { peer_count, .. } => {
            ...
            lines.push(Line::from(vec![
                Span::styled(" remote ", Style::default().fg(UPPLI_ACCENT)),
    ...
    if let Some(url) = app.remote_session_url.as_deref() {
        lines.push(Line::from(vec![
            Span::styled(" link ", Style::default().fg(UPPLI_ACCENT)),
            Span::styled(
    ...
    // "Note: You have launched uppli-code in your home directory. For the best
    ...
    if app.home_dir_warning {
        lines.push(Line::from(vec![
            ...
            Span::styled(
                truncate_end(
                    "You have launched uppli-code in your home directory. \
...
pub fn render_app(frame: &mut Frame, app: &App) {
    ...
    // Invalid config/settings dialog (shown when settings.json or UPPLI.md is malformed)
...
fn render_welcome_box(frame: &mut Frame, app: &App, area: Rect) {
    ...
    // Outer border with title "Uppli Code vX.Y"
    let outer_block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .border_style(Style::default().fg(UPPLI_ACCENT))
        .title(Line::from(vec![
            Span::styled(
                " Uppli Code ",
                Style::default()
    ...
    let divider_lines: Vec<Line> = (0..inner.height)
        .map(|_| Line::from(Span::styled("\u{2502}", Style::default().fg(UPPLI_ACCENT))))
        .collect();
    ...
    let tip_text = cc_core::tips::select_tip(0)
        ...
        .unwrap_or_else(|| {
            "Run /init to create a UPPLI.md file with project instructions".to_string()
        });
