...
// (e.g. ~/.uppli/UPPLI.md or a project-local UPPLI.md).
//
...
mod tests {
    ...
    fn memory_notif_show_and_dismiss() {
        ...
        state.show("/home/user/.uppli/UPPLI.md");
        assert!(state.visible);
        assert_eq!(state.memory_path, "/home/user/.uppli/UPPLI.md");
        state.dismiss();
        assert!(!state.visible);
    }
    ...
    fn memory_notif_can_reshown_after_dismiss() {
        let mut state = MemoryUpdateNotificationState::new();
        state.show("/tmp/UPPLI.md");
        state.dismiss();
        assert!(!state.visible);
        state.show("/tmp/other/UPPLI.md");
        assert!(state.visible);
        assert_eq!(state.memory_path, "/tmp/other/UPPLI.md");
    }
