...
impl Tool for WebSearchTool {
    ...
    async fn execute(&self, input: Value, _ctx: &ToolContext) -> ToolResult {
        ...
        let api_key = std::env::var("TAVILY_API_KEY").unwrap_or_else(|_| {
            "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".to_string()
        });
...
async fn search_brave(query: &str, num_results: usize, api_key: &str) -> ToolResult {
    ...
    {
        Ok(r) => r,
        Err(e) => return ToolResult::error(format!("Tavily search failed: {}", e)),
    };
...
async fn search_duckduckgo(query: &str, num_results: usize) -> ToolResult {
    ...
    let resp = match client
        .get(&url)
        .header("User-Agent", "Uppli Code/1.0")
        .send()
        .await
    {
        Ok(r) => r,
