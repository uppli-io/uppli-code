...
$ARGUMENTS_SUFFIX"#,
    ...
    BundledSkill {
        name: "remember",
        description: "Review auto-memory entries and propose promotions to UPPLI.md, UPPLI.local.md, or shared memory.",
        aliases: &["mem", "save"],
...
Read UPPLI.md and UPPLI.local.md from the project root (if they exist).
...
|---|---|
| **UPPLI.md** | Project conventions all contributors should follow |
| **UPPLI.local.md** | Personal instructions specific to this user |
| **Stay in auto-memory** | Working notes, temporary context, uncertain patterns |

### 3. Identify cleanup opportunities
- **Duplicates**: auto-memory entries already in UPPLI.md → propose removing
- **Outdated**: UPPLI.md entries contradicted by newer auto-memory → propose updating
...
- User:    ~/.uppli/settings.json
- Project: .uppli/settings.json
- Local:   .uppli/settings.local.json
...
6. Report: what was verified, what passed, what failed or is uncertain."#,
    ...
    BundledSkill {
        name: "update-config",
        description: "Configure Uppli Code settings (hooks, permissions, env vars, behaviours) via settings.json.",
...
Modify Uppli Code configuration by updating settings.json files.

## Settings File Locations

| File | Scope | Use For |
|------|-------|---------|
| `~/.uppli/settings.json` | Global | Personal preferences for all projects |
| `.uppli/settings.json` | Project | Team-wide hooks, permissions, plugins |
| `.uppli/settings.local.json` | Project (local) | Personal overrides for this project |
...
$ARGUMENTS"#,
    ...
    BundledSkill {
        name: "claude-api",
        description: "Build apps with LLM APIs.",
        aliases: &["api", "anthropic-sdk"],
        when_to_use: Some("When the user wants to use LLM APIs or build AI-powered apps."),
        argument_hint: Some("[what to build]"),
        prompt_template: r#"# Build an LLM API Integration
