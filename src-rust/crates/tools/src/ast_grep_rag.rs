...
fn builtin_patterns() -> Vec<AstGrepPattern> {
    vec![
        ...
        p(
            "Match raise ValueError specifically",
        ...
        p(
            "Add line after a function call",
            "$VAR = $OBJ.$METHOD($$$ARGS)",
            "$VAR = $OBJ.$METHOD($$$ARGS)\nnew_statement_here",
            "python",
        ...
        p(
            ...
            "re.compile($ARG)",
            "re.compile($ARG, re.IGNORECASE)",
            "python",
            &["argument", "add", "flag", "compile", "IGNORECASE", "case"],
        ),
        p(
            "Match .format() call on string",
            "$STR.format($$$ARGS)",
            "",
            "python",
        ...
        p(
            "Match any function call with wildcards",
            "$FUNC($$$ARGS)",
            "",
            "any",
