// ConfigTool: get or set Uppli Code configuration settings at runtime.
//
// Reads from and persists to ~/.uppli/settings.json.
...
static SUPPORTED_SETTINGS: &[(&str, &str)] = &[
    (
        "model",
        "LLM model to use (e.g. 'deepseek-reasoner', 'deepseek-chat')",
    ),
...
impl Tool for ConfigTool {
    ...
    fn description(&self) -> &str {
        "Get or set Uppli Code configuration settings. Omit 'value' to read the current value. \
         Supported settings: model, max_tokens, verbose, permission_mode, auto_compact. \
         Changes persist to ~/.uppli/settings.json."
    }
