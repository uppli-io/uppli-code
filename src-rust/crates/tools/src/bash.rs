...
use cc_core::bash_classifier::{classify_bash_command, BashRiskLevel};
use cc_core::tasks::{global_registry, BackgroundTask};
use regex::Regex;
                    ...
                    match c.wait().await {
                        ...
                        Ok(status) => {
                            ...
                            global_registry().update_status(
                                &task_id_clone,
                                cc_core::tasks::TaskStatus::Failed(format!("exit code {}", code)),
                            );
                        }
                        Err(e) => {
                            global_registry().update_status(
