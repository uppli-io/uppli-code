...
// Durable tasks are persisted to `~/.uppli/scheduled_tasks.json`.
//
...
/// Path to `~/.uppli/scheduled_tasks.json`.
fn scheduled_tasks_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".uppli").join("scheduled_tasks.json"))
}
...
async fn ensure_store_loaded() {
    ...
    *init = true;
    ...
    // Load from ~/.uppli/scheduled_tasks.json if it exists.
...
impl Tool for CronCreateTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "durable": {
                    "type": "boolean",
                    "description": "true = persist to .uppli/scheduled_tasks.json; false (default) = session only"
                }
            },
    ...
    async fn execute(&self, input: Value, _ctx: &ToolContext) -> ToolResult {
        ...
        // Persist to ~/.uppli/scheduled_tasks.json for durable tasks.
        if params.durable {
        ...
        let where_note = if params.durable {
            "Persisted to ~/.uppli/scheduled_tasks.json"
        } else {
            "Session-only (dies when Uppli Code exits)"
        };
...
/// Persist all durable tasks to `~/.uppli/scheduled_tasks.json`.
