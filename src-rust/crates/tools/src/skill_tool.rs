...
//   <project>/.uppli/commands/<name>.md
//   ~/.uppli/commands/<name>.md
//
...
impl Tool for SkillTool {
    ...
    fn description(&self) -> &str {
        "Execute a skill (custom prompt template) by name. \
         Skills are .md files in .uppli/commands/ or ~/.uppli/commands/. \
         Use skill=\"list\" to discover available skills. \
...
fn skill_search_dirs(ctx: &ToolContext) -> Vec<PathBuf> {
    let mut dirs = vec![ctx.working_dir.join(".uppli").join("commands")];
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".uppli").join("commands"));
    }
    dirs
}
...
async fn list_skills(dirs: &[PathBuf]) -> ToolResult {
    ...
    if total == 0 {
        return ToolResult::success(
            "No skills found. Create .md files in .uppli/commands/ to define skills.\n\
