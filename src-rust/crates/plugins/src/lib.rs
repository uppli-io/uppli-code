// cc-plugins: Plugin runtime for the Uppli Code CLI Rust port.
//
...
///
/// Search order:
/// 1. `~/.uppli/plugins/`  (user-global)
/// 2. `<project_dir>/.uppli/plugins/`  (project-local)
...
///
/// Copies the plugin directory into `~/.uppli/plugins/` and returns the
