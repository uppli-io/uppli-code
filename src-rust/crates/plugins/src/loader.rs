...
/// 1. `~/.uppli/plugins/<name>/`  — user-global plugins
/// 2. `<project>/.uppli/plugins/<name>/`  — project-local plugins
...
/// Return the default user-level plugins directory: `~/.uppli/plugins`.
pub fn default_user_plugins_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".uppli").join("plugins"))
}

/// Return the project-level plugins directory: `<project>/.uppli/plugins`.
pub fn project_plugins_dir(project_dir: &Path) -> PathBuf {
    project_dir.join(".uppli").join("plugins")
}
