...
/// Scan `project_root` for skill definitions in `.uppli/skills/` and the bundled
...
pub async fn prefetch_skills(project_root: &Path, index: SharedSkillIndex) {
    ...
    // 1. User-defined skills: ~/.uppli/skills/*.md + {project_root}/.uppli/skills/*.md
    let search_dirs: Vec<std::path::PathBuf> = {
        let mut dirs = Vec::new();
        if let Some(home) = dirs::home_dir() {
            dirs.push(home.join(".uppli").join("skills"));
        }
        dirs.push(project_root.join(".uppli").join("skills"));
        dirs
    };
