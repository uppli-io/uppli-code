...
// remembering and persist them to UPPLI.md.
//
...
//   3. Append entries under "## Auto-extracted memories" in UPPLI.md
...
impl SessionMemoryExtractor {
    ...
    pub async fn extract(
        ...
        messages: &[Message],
        working_dir: &Path,
        api_client: &dyn cc_api::LlmProvider,
...
MEMORY: code_pattern | 7 | Uses builder pattern";
    ...
    async fn test_persist_creates_file() {
        ...
        let target = dir.path().join(".uppli").join("UPPLI.md");
    ...
    async fn test_persist_appends_to_existing() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("UPPLI.md");
