...
fn session_dir_encoding() {
    ...
    // Should contain the base64-encoded project root under .uppli/projects/.
...
fn load_memory_from_nonexistent_dir() {
    // Loading from a dir with no UPPLI.md should return empty, not panic.
    let tmp = TempDir::new().unwrap();
    let files = load_all_memory_files(tmp.path());
    // May be empty or may pick up user ~/.uppli/UPPLI.md — both are valid.
    let _ = files;
}
