...
//   - Entries are written to `~/.uppli/history.jsonl` via O_APPEND.
...
//     `~/.uppli/pastes/<sha256-hex>` and referenced by hash.
...
/// Persist `text` to `~/.uppli/pastes/<hash>`.  Fire-and-forget.
...
/// Read text from `~/.uppli/pastes/<hash>`, returning `None` if missing.
...
/// Append `entry` to `~/.uppli/history.jsonl`.
///
