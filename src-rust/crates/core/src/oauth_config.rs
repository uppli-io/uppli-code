...
// ---------------------------------------------------------------------------
// OAuth configs — intentionally empty.
// Uppli Code uses API keys, not OAuth.  These structs are kept because
// other modules reference the type, but every URL is blank so no
// external request can ever be made.
// ---------------------------------------------------------------------------
...
pub const STAGING_OAUTH: OAuthConfig = PROD_OAUTH;

/// Client-ID Metadata Document URL for MCP OAuth (CIMD / SEP-991).
/// MCP Client-ID Metadata URL — disabled (was claude.ai-specific).
...
mod tests {
    ...
    #[test]
    fn test_oauth_configs_are_disabled() {
        // Uppli Code uses API keys — OAuth URLs must be empty.
        assert!(PROD_OAUTH.token_url.is_empty());
        assert!(PROD_OAUTH.api_key_url.is_empty());
        assert!(PROD_OAUTH.client_id.is_empty());
        assert!(STAGING_OAUTH.token_url.is_empty());
    }
