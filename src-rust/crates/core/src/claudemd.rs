//! UPPLI.md hierarchical memory loading.
...
pub enum MemoryScope {
    /// `~/.uppli/rules/*.md` — global managed policy.
    Managed,
    /// `~/.uppli/UPPLI.md` — user-level memory.
    User,
    /// `{project_root}/UPPLI.md` — project-level memory.
    Project,
    /// `{project_root}/.uppli/UPPLI.md` — local override.
    Local,
}

/// Frontmatter parsed from a UPPLI.md file.
...
/// Load all UPPLI.md files for the given project root, in priority order.
///
...
pub fn load_all_memory_files(project_root: &Path) -> Vec<MemoryFileInfo> {
    ...
    if let Some(home) = dirs::home_dir() {
        let rules_dir = home.join(".uppli/rules");
