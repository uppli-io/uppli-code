...
impl NamedCommand for AgentsCommand {
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        match args.first().copied().unwrap_or("list") {
            "list" => {
                // Load agent definitions from .uppli/agents/ in working dir
            ...
            "create" => {
                ...
                CommandResult::Message(format!(
                    "Create a new agent by adding .uppli/agents/{name}.md\n\
                     Template:\n\
                     ---\n\
                     name: {name}\n\
            ...
            "edit" => {
                ...
                CommandResult::Message(format!(
                    "Edit .uppli/agents/{name}.md in your editor to update the agent."
                ))
            }
            "delete" => {
                let name = match args.get(1).copied() {
                ...
                CommandResult::Message(format!(
                    "Delete .uppli/agents/{name}.md to remove the agent."
                ))
            }
...
impl NamedCommand for AddDirCommand {
    ...
    fn description(&self) -> &str {
        "Add a directory to Uppli Code's allowed workspace paths"
    }
    fn usage(&self) -> &str {
        "claude add-dir <path>"
    }
                        ...
                        if session.tags.iter().any(|t| t == &tag) {
                            ...
                            let remove_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(
                                    cc_core::history::untag_session(&session_id, &tag_clone),
                                )
                            });
                            match remove_result {
                                Ok(()) => CommandResult::Message(format!("Removed tag: #{tag}")),
                            ...
                            let add_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(
                                    cc_core::history::tag_session(&session_id, &tag_clone),
                                )
                            });
                            match add_result {
                                Ok(()) => CommandResult::Message(format!("Added tag: #{tag}")),
...
impl NamedCommand for PassesCommand {
    ...
    fn description(&self) -> &str {
        "Share a free week of Uppli Code with friends"
    }
    fn usage(&self) -> &str {
        "claude passes"
    }
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        CommandResult::Message(
            "Uppli Code Passes \u{2014} Share Uppli Code with friends\n\n\
             Share a free week of Uppli Code with a friend\n\
             Visit https://uppli.dev/passes to get your referral link\n\
             Each referral gives your friend 1 week of Uppli Code Pro"
                .to_string(),
        )
    }
}
        ...
        if ides.is_empty() {
            CommandResult::Message(
                "No IDE connections detected.\n\
                 To connect an IDE, install the Uppli Code extension in VS Code or JetBrains."
                    .to_string(),
            )
        } else {
            CommandResult::Message(format!(
...
impl NamedCommand for DesktopCommand {
    ...
    fn description(&self) -> &str {
        "Download Uppli Code desktop app"
    }
    fn usage(&self) -> &str {
        "claude desktop"
    }
    ...
    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        if let Some(ref session_url) = ctx.remote_session_url {
            ...
            msg.push_str("Your Uppli Code session is synced with Claude Desktop.\n\n");
        ...
        let msg = if os == "macos" {
            if desktop_likely_installed {
                format!(
                    "Open Claude Desktop \u{2014} macOS\n\n\
                     ...
                     Launch it from /Applications/Uppli.app and sign in with your Anthropic account.\n\n\
                ...
                format!(
                    "Download Claude Desktop \u{2014} macOS\n\n\
                     ...
                     3. Uppli Code will detect the Desktop bridge automatically"
                )
            }
        } else if os == "windows" {
            ...
            format!(
                "Claude Desktop is not yet available for {os}\n\n\
                 On Linux, you can use Uppli Code via the CLI or visit https://uppli.dev in your browser.\n\
...
impl NamedCommand for MobileCommand {
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        let ios_url = "https://uppli.dev/mobile/ios";
        let android_url = "https://uppli.dev/mobile/android";
        let mobile_url = "https://uppli.dev/mobile";
        ...
        let session_qr_url: String = if let Some(ref url) = ctx.remote_session_url {
            ...
            format!("https://uppli.dev/code/mobile?session={}", encoded)
        } else {
            String::new()
        };
...
impl NamedCommand for InstallGithubAppCommand {
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        CommandResult::Message(
            "To install the Uppli Code GitHub App:\n\
             ...
             The app enables Uppli Code in GitHub Actions workflows.\n\
             Docs: https://docs.uppli.dev/github-actions"
                .to_string(),
        )
    }
}
...
impl NamedCommand for RemoteSetupCommand {
    ...
    fn description(&self) -> &str {
        "Check and configure a remote Uppli Code environment"
    }
    fn usage(&self) -> &str {
        "claude remote-setup"
    }
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        ...
        CommandResult::Message(format!(
            ...
            steps.join("\n"),
            if all_ok {
                "\u{2713} All checks passed. Uppli Code is ready for remote use.\nStart a session: claude --bridge"
            } else {
...
impl NamedCommand for StickersCommand {
    ...
    fn description(&self) -> &str {
        "Open the Uppli Code sticker page in your browser"
    }
    fn usage(&self) -> &str {
        "claude stickers"
    }
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        let url = "https://uppli.dev/stickers";
        match open::that(url) {
