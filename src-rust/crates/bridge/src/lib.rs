...
//
// The bridge connects the local Uppli Code CLI to the uppli.dev web UI,
...
pub struct BridgeConfig {
    ...
    pub enabled: bool,
    /// Base URL for bridge API calls (e.g. `https://uppli.dev`).
...
impl Default for BridgeConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            server_url: String::new(), // Disabled by default — set UPPLI_BRIDGE_URL to enable
...
impl BridgeConfig {
    ...
    /// - `UPPLI_BRIDGE_URL` — overrides `server_url` and sets `enabled = true`
    /// - `UPPLI_BRIDGE_TOKEN` / `UPPLI_BRIDGE_OAUTH_TOKEN` — sets `session_token`
    /// - `UPPLI_BRIDGE_BASE_URL` — alternative URL override (ant-only dev override)
    pub fn from_env() -> Self {
        let mut config = Self::default();

        // URL override (sets enabled implicitly)
        if let Ok(url) =
            std::env::var("UPPLI_BRIDGE_URL").or_else(|_| std::env::var("UPPLI_BRIDGE_BASE_URL"))
        {
            if !url.is_empty() {
        ...
        // Token override
        if let Ok(token) = std::env::var("UPPLI_BRIDGE_TOKEN")
            .or_else(|_| std::env::var("UPPLI_BRIDGE_OAUTH_TOKEN"))
        {
            if !token.is_empty() {
...
/// 1. `UPPLI_BRIDGE_TOKEN` environment variable
/// 2. `UPPLI_BRIDGE_OAUTH_TOKEN` environment variable
///
...
pub async fn start_bridge_session(
    ...
    let token = token_override
        .or_else(|| std::env::var("UPPLI_BRIDGE_TOKEN").ok())
        .or_else(|| std::env::var("UPPLI_BRIDGE_OAUTH_TOKEN").ok())
        .filter(|t| !t.is_empty())
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Remote Control requires a session token.\n\
                 Set UPPLI_BRIDGE_TOKEN=<your-token> to enable.\n\
                 Get a token from https://uppli.dev (Settings → Remote Control).\n\
                 Note: Remote Control is only available with uppli.dev subscriptions."
            )
        })?;

    // Resolve server base URL.
    let server_url = std::env::var("UPPLI_BRIDGE_URL")
        .or_else(|_| std::env::var("UPPLI_BRIDGE_BASE_URL"))
        ...
        401 | 403 => {
            anyhow::bail!(
                "Bridge session registration failed: authentication error (HTTP {}).\n\
                 ...
                 Get a new token from https://uppli.dev (Settings → Remote Control).",
                status
            );
        }
        404 => {
...
mod tests {
    ...
    fn test_jwt_decode_invalid() {
        ...
        let _ = JwtClaims::decode("only.two"); // must not panic
    }
